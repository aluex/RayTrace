...
int obj_parse_mtl_file(char *filename, list *material_list)
{
	...
	while( fgets(current_line, OBJ_LINE_SIZE, mtl_file_stream) )
	{
		...
		else if( strequal(current_token, "d") && material_open)
		{
            ...
            current_mtl->refract = 1.0 - current_mtl->trans;
		}
		//reflection
...
int obj_parse_obj_file(obj_growable_scene_data *growable_data, char const *filename)
{
...
int parse_obj_scene(obj_scene_data *data_out, char const *filename)
{
